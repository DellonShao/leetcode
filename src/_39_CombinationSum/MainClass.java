...
class Solution {
    ...
    private void dfs(int[] candidates, int target, List<List<Integer>> result, LinkedList<Integer> cur, int index){
        if(target == 0){
            Object tmp = cur.clone();
            //cur = (LinkedList<Integer>) tmp;
        ...
        for(int i = index; i < candidates.length; ++i){
            ...
            target -= candidates[i];
            dfs(candidates, target, result, cur, i);
            cur.removeLast();
            target += candidates[i];
        }
    }
